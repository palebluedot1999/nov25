"""
Strategy 1: Top Holdings, Equal Weight — Drill-Down
...
for variant_tab, stream_id, variant_label in [
    ...
    with variant_tab:
        ...
        with sub2:
            ...
            # Show what the latest transactions were as a proxy
        ...
        with sub3:
            if perf.empty:
                ...
                fig.add_trace(go.Scatter(
                    x=perf["date"],
                    y=perf["portfolio_return"] * 100,
                    name=f"{stream_id} Portfolio",
                    line=dict(color="#1f77b4", width=2),
                ))
                fig.add_trace(go.Scatter(
                    x=perf["date"],
                    y=perf["xbi_return"] * 100,
                    name="XBI Benchmark",
