...
| **`security_identifiers.csv`** *(new, §5.1)* | `utils/security_reference.py` | `cusip, ticker, name, figi, composite_figi, share_class_figi, security_type, market_sector, exch_code, source, resolved_at` | Append/upsert-by-CUSIP resolver output cache. `source ∈ {openfigi, company_tickers}`. |

### 3.2 `data/processed/` — derived tables (regenerated by `scripts/consolidate_*.py` / build scripts)

| Path | Producer | Schema | Notes |
|---|---|---|---|
| `holdings.csv` | `scripts/consolidate_holdings.py` → `utils/holdings_operations.py` | **current:** `portfolio, ticker, cusip, shares, eod_date` — **after this change (§6):** `portfolio, security_id, cusip, ticker, name, shares, eod_date` | ~122K daily rows (forward-filled from quarterly filings). Currently drops unresolved-ticker positions; §6 removes that. |
...
| **`security_reference.csv`** *(new, §5.2)* | `scripts/build_security_reference.py` → `utils/security_reference.py` | `cusip, ticker, name, isin, figi, composite_figi, security_type, market_sector, exch_code, is_active, first_seen_quarter, last_seen_quarter, n_filings, resolution_source, resolution_status, built_at` | One row per distinct CUSIP in any 13F filing. The joinable master. |
...
| **`config/security_overrides.csv`** *(new, §5.3)* | `cusip, ticker, name, security_type, note` | Hand-maintained identifier overrides. **Committed to git** — highest precedence in the merge. |
...
- **Daily consolidated path** — `utils/csv_data.py::load_processed_holdings`
  reads `processed/holdings.csv`. This is where the drop happens.
  Consumer: `dashboard/pages/1_Dashboard.py` (Portfolio Size / value-over-time).
...
data/raw/security_identifiers.csv <── resolve_cusips()  ── OpenFIGI bulk  ──┐
        ...
        data/processed/security_reference.csv   (+ coverage report to docs/)
                            ▼
   utils/holdings_operations.py  ── resolve tickers, keep-by-cusip, has_price flag
                            ▼
...
- Columns: `cusip, ticker, name, figi, composite_figi, share_class_figi,
  security_type, market_sector, exch_code, source, resolved_at`.
- `source ∈ {openfigi, company_tickers}`.
...
- Columns:
  ...
  | `isin` | derived | `"US" + cusip + check_digit` (Luhn mod-10 over the 11 alphanumerics); empty if `cusip` malformed |
...
- Columns: `cusip, ticker, name, security_type, note`.
...
- `cusip_to_isin(cusip: str) -> str`
  ISIN check-digit derivation; returns `""` on malformed input.
- `resolve_cusips(cusips: list[str], *, force=False) -> pd.DataFrame`
  ...
  1. OpenFIGI bulk (reuse `utils/cusip_mapping.CUSIPMapper`, extended to keep
     the full response, not just `ticker`), batched to API limits.
...
1. `resolve_tickers_from_cusip_cache()` → rename to
   ...
   `data/processed/security_reference.csv` (`cusip → ticker, name`), falling
   back to `data/raw/cusip_cache.csv` if the reference file does not exist yet.
2. **Remove the drop** at `~:174`
   (`valid_holdings = filing_df[filing_df['ticker'].notna()]`). Every position
   is carried, with:
   - `security_id` = `cusip` (stable key, always present)
   - `ticker` = resolved ticker or `""`
   - `name` = resolved / filing issuer name
3. Position exits (`detect_position_exits`) key on `security_id`, not `ticker`
   (so a blank-ticker exit is still detected).
4. Price join (`calculate_position_values`): left-join on `ticker`; add
   `has_price: bool`; `position_value = shares * close` where `has_price` else
   `NaN`.
5. `processed/holdings.csv` schema →
   `portfolio, security_id, cusip, ticker, name, shares, eod_date`.
   The price-joined position-value frame additionally carries
   `has_price, close, position_value`.
   `cusip` and `ticker` columns are **retained** for backward compatibility with
   current readers.
...
|---|---|---|
| `utils/csv_data.py::load_processed_holdings` | `holdings.csv` | tolerate new columns; update docstring (currently promises `portfolio, ticker, cusip, shares, eod_date`). |
| `dashboard/pages/1_Dashboard.py` (Portfolio Size, value-over-time) | `load_processed_holdings` + prices | sum `position_value` with `.fillna(0)`; show a caption "N positions unpriced (X% of value gap)". |
...
3. Triage the non-`resolved` worklist into `config/security_overrides.csv`;
   ...
   `resolved`).
4. Change `utils/holdings_operations.py` (§6); regenerate `holdings.csv` via
   `scripts/consolidate_holdings.py`. Verify direction: more unique securities,
   `position_value` total rises by the now-included priced names, unpriced
   count is plausible.
...
- `resolve_cusips` with **mocked** OpenFIGI + **mocked** `company_tickers.json`:
  ...
  name match → `source=company_tickers, resolution_status=name_only`; both miss →
  not written to cache, appears as `unresolved` in `build_security_reference`.
- **Regression for the drop removal:** a filing containing an unresolvable CUSIP
  → after `process_quarterly_filings_to_daily_holdings`, that `security_id` is
  present in the output with `ticker == ""`, `has_price == False`,
  `position_value` NaN.
...
- **Debt CUSIPs** (e.g. `00484MAA4`) — OpenFIGI may resolve these as bonds with
  no tradable equity ticker / no Yahoo price → `has_price = False`. Expected and
  documented; they still appear in the reference table and holdings.
